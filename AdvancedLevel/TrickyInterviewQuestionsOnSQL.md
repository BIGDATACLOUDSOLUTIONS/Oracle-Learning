...
If Yes:
  Answer: select d.DeptName,nvl(max(e.salary),0) as max_sal from department d left outer join employee e on e.DeptId=d.id group by d.DeptName;
If No:
  Answer: select d.DeptName,nvl(max(e.salary),0) as max_sal from department d inner join employee e on e.DeptId=d.id group by d.DeptName;
```
